...
export class ToolSelector {
  ...
  enhanceToolWithUsageGuidance(schema: OpenAITool): OpenAITool {
    ...
    this.logger.debug(`DEBUG: Enhancing tool ${schema.function.name}, guidance found: ${guidance ? 'YES' : 'NO'}`);
    ...
    this.logger.debug(`DEBUG: Enhanced ${schema.function.name} description: ${enhanced.function.description}`);
    return enhanced;
  }
  ...
  async selectToolWithLLM(messages: any[], tools: OpenAITool[]): Promise<ToolSelectionResult | null> {
    ...
    try {
      ...
      const toolResponse = await this.ollamaClient.sendToOllama(toolSelectionPrompt, stage1Config.temperature, stage1Config.maxTokens, stage1Config.useFastModel);
      ...
      try {
        ...
        this.logger.error(`Failed to parse tool selection JSON: ${parseError}`, { response: cleanToolResponse });
        return null;
      }
      ...
      if (this.isSimpleArgumentGeneration(toolSelection.tool)) {
        this.logger.info('Using fast model for simple argument generation', { tool: toolSelection.tool });
        argsSelection = await this.generateArgsWithFastModel(userRequest, selectedTool);
      } else {
        this.logger.info(`🧠 Using full model for complex argument generation: ${toolSelection.tool}`);
  ...
  private async generateArgsWithFastModel(userRequest: string, toolSchema: OpenAITool): Promise<any> {
    ...
    const argsResponse = await this.ollamaClient.sendToOllama(argsPrompt, fastArgsConfig.temperature, undefined, fastArgsConfig.useFastModel);
  ...
  private async generateArgsWithFullModel(userRequest: string, toolSchema: OpenAITool): Promise<any> {
    ...
    const argsResponse = await this.ollamaClient.sendToOllama(argsPrompt, fullArgsConfig.temperature, undefined, fullArgsConfig.useFastModel);
    const cleanArgsResponse = argsResponse.trim().replace(/```json|```/g, '').trim();
